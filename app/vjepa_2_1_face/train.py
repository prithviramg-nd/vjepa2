...
from app.vjepa_2_1_face.mlflow_utils import MLflowRun
from app.vjepa_2_1_face.utils import (
    init_opt,
    init_video_model,
    load_checkpoint,
    normalize_nested,
...
def main(args, resume_preempt=False):
    ...
    # -- LOW-DIM OUTPUT HEAD
    # The encoder projects every token down to `output_dim` (e.g. 8). During
    # training it emits `levels_predictor * output_dim` (4 levels concatenated);
    # at inference it emits exactly `output_dim`, i.e. [T/tub, H/p, W/p, output_dim].
    # `embed_dim_encoder` is the PER-LEVEL width used to chunk/LayerNorm targets.
    output_dim = cfgs_model.get("output_dim", 8)
    head_hidden_ratio = cfgs_model.get("head_hidden_ratio", 0.0)
    embed_dim_encoder = output_dim
    ...
    # -- MLflow (rank 0 only; every call is a safe no-op on failure)
    mlf = MLflowRun(args.get("mlflow"), rank=rank)
    mlf.log_params(
        {
            k: v
            for k, v in args.items()
            if k in ("model", "data", "data_aug", "loss", "optimization", "meta", "folder", "app")
        }
    )
    mlf.log_params({"runtime.world_size": world_size})
    mlf.set_tag("output_dim", output_dim)
    mlf.set_tag("model_name", model_name)
    mlf.set_tag("crop_size", crop_size)
    ...
    mlf.log_params({"runtime.ipe": ipe, "runtime.dataset_batches": _dlen})
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        mlf.log_metrics({"epoch_loss": loss_meter.avg}, step=(epoch + 1) * ipe)
        if (epoch + 1) % CHECKPOINT_FREQ == 0 or epoch == (num_epochs - 1):
            ...
            if save_every_freq > 0 and (epoch + 1) % save_every_freq == 0:
                ...
                mlf.log_artifact(save_every_path, artifact_path="checkpoints")

    mlf.log_artifact(log_file, artifact_path="logs")
    mlf.close()
